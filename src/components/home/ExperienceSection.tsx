    ...
    <section id="experience" className="py-8 md:py-12 pb-20 px-6 bg-secondary/10">
      <div className="container max-w-6xl mx-auto space-y-6">
          <div className="text-center space-y-1">
            ...
            <h3 className="text-4xl md:text-5xl font-black tracking-tighter">The Technical Stack</h3>
          </div>
          ...
          <Tabs.Root value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <Tabs.Content key="work-content" value="work" className="grid grid-cols-1 md:grid-cols-2 gap-2 outline-none">
              {experience.map((work, i) => (
                <motion.div key={work.company} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
                  <Card className="h-full border border-border/40 shadow-sm hover:shadow-xl hover:scale-[1.02] transition-all duration-500 rounded-3xl group overflow-hidden bg-background">
                    <CardContent className="p-10 space-y-6">
                      <div className="flex justify-between items-start">
                        ...
                        <Badge className="bg-secondary text-primary border-none rounded-lg font-black text-[9px] uppercase tracking-widest">{work.period}</Badge>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2 pt-4">
                        {work.tags.map(tag => (
                          <span key={tag} className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60 border border-border/60 px-3 py-1 rounded-full">{tag}</span>
                        ))}
                      </div>
                    </CardContent>
                  </Card>
            ...
            <Tabs.Content key="projects-content" value="projects" className="grid grid-cols-1 md:grid-cols-2 gap-2 outline-none">
              {projects.map((proj, i) => (
                <motion.div key={proj.name} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.1 }}>
                  <Card className="group border border-border/40 rounded-3xl overflow-hidden bg-background h-full hover:shadow-2xl transition-all duration-500">
                    <div className="h-12 bg-muted group-hover:bg-accent/5 transition-colors relative flex items-center justify-center p-2">
                      <div className="absolute top-1 left-1"><span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40">{proj.type}</span></div>
                    ...
                    <CardContent className="p-2.5 space-y-1">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                          <Link href={proj.links.git} className="p-2 rounded-xl bg-secondary hover:bg-accent hover:text-white transition-colors"><Github className="h-4 w-4" /></Link>
                          <Link href={proj.links.live} className="p-2 rounded-xl bg-secondary hover:bg-accent hover:text-white transition-colors"><ExternalLink className="h-4 w-4" /></Link>
                        </div>
                      </div>
            ...
            <Tabs.Content key="skills-content" value="skills" className="grid grid-cols-1 md:grid-cols-3 gap-2 outline-none">
              {skills.map((cat, i) => (
                <motion.div key={cat.name} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}>
                  <Card className="group border border-border/40 rounded-3xl bg-background hover:border-accent/40 transition-all duration-500 h-full p-10 space-y-10">
                    <div className="flex items-center gap-2">
                      <div className="h-14 w-14 rounded-2xl bg-secondary flex items-center justify-center text-primary"><cat.icon className="h-6 w-6 group-hover:text-accent transition-colors" /></div>
                ...
                <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
                  <Card className="border border-border/40 rounded-3xl bg-background p-10 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:shadow-lg transition-all group">
                    <div className="flex items-center gap-6">
                      <div className="h-14 w-14 rounded-2xl bg-secondary flex items-center justify-center text-primary group-hover:bg-accent group-hover:text-white transition-all"><GraduationCap className="h-6 w-6" /></div>
                    ...
                    <div className="text-right">
                      <Badge className="bg-secondary text-primary border-none rounded-lg font-black text-[9px] uppercase tracking-widest mb-2">{edu.period}</Badge>
                      <p className="text-muted-foreground/60 text-[10px] font-black uppercase tracking-widest">{edu.honors}</p>
                    </div>
                  </Card>
                </motion.div>
              ))}
